		...
		(function(proceed) {
			...
			for (var i = 0; i < plan.length; ++i) {
				...
				if (speed) { // speed/dir mode
					...
					switch (p.dir) {
						case 'up':
							p.from = { xrel: .5, yrel: 0, zoom: 1.1 };
							p.to = { xrel: .5, yrel: .3, zoom: 1.1 }; // cambio yrel: 1 por 0.5 
							var slide_px = p.height - self_height - 2 * fade_px;
							break;
						case 'down':
							p.from = { xrel: .5, yrel: .3, zoom: 1.1 };// cambio yrel: 1 por 0.5
							p.to = { xrel: .5, yrel: 0, zoom: 1.1 };
							var slide_px = p.height - self_height - 2 * fade_px;
							break;
						case 'left':
